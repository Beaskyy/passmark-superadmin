...
import { columns, User as UIUser } from "@/app/(root)/users/columns" // Reusing columns from users
import { DataTable } from "@/components/ui/data-table"
import { DataTableToolbar } from "@/app/(root)/users/data-table-toolbar" // Reusing toolbar
import { Plus, Upload, Loader2, AlertCircle } from "lucide-react"
import { useActiveUsersList } from "@/hooks/use-active-users-list"
import { ActiveUser as ApiActiveUser } from "@/types/api"
...
const ActiveUsers = () => {
    ...
    const { data: usersData, isLoading, isError, error } = useActiveUsersList({
        page: pagination.pageIndex + 1, // API uses 1-based indexing
        limit: pagination.pageSize
    })

    const mapApiUserToUiUser = (user: ApiActiveUser): UIUser => {
        ...
        return {
            ...
            avatarUrl: undefined, 
            // active users specific data could go here if columns were different
        }
    }
    ...
    <main className="text-white">
      <div className="flex flex-col gap-2">
           <div className="flex md:flex-row flex-col items-center gap-6 justify-between mb-2">
            ...
            <h2 className="md:text-[30px] text-xl font-bold">Active User Management</h2>
             <p className="text-[#94A3B8] text-sm">View and manage currently active users.</p>
            </div>
           ...
           {isLoading ? (
               ...
               <div className="flex h-[400px] flex-col items-center justify-center gap-2 rounded-md border border-red-900/50 bg-[#0F172A] text-red-500">
                   ...
                   <p>Error loading active users: {error?.message || "Unknown error"}</p>
